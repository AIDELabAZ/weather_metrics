...
# Exact system prompt from finetune_claude.py — must match inference verbatim
SYSTEM_PROMPT = (
...
# Question texts copied verbatim from finetune_claude.py
QUESTIONS = [
    {
        "key": "Title",
        "question": (
            "Task: Extract the article title.\nLook: top of first page (main heading before authors/abstract).\n"
            "Rules: include subtitle if in the same heading; exclude authors/affiliations/journal headers/footers/section headers; "
            "ignore footnote markers (*, †, superscripts).\nOutput: ONE line: title text only. No quotes, labels, or extra words."
        ),
    },
    {
        "key": "DOI",
    ...
    },
    {
        ...
        "question": (
            "Task: Does the article contain empirical quantitative statistical estimation "
            "(e.g., regressions/econometrics with estimated coefficients/SEs/p-values)?\n"
            "Look: 'we estimate/regress', model equations with error terms, regression tables with coefficients.\n"
            "Rules: do NOT infer from topic/title/abstract. Count only analysis done in THIS article (not summaries of other papers). "
            "Exclude purely theoretical work, qualitative/descriptive only, and simulation/calibration only.\n"
    ...
    },
    {
        ...
        "question": (
            ...
            "Rules: exclude first-stage outcomes, RHS variables (treatments/endogenous regressors/instruments/controls/covariates/fixed effects), "
    ...
    },
    {
        ...
        "question": (
            ...
            "Part A (binary): Task: In the MAIN empirical analysis, do the authors treat any RHS variable as endogenous "
            "(correlated with the error term) and address it explicitly?\n"
            "Look: statements that a regressor is endogenous + a method to address it "
            "(IV/2SLS/3SLS/LIML, control function/2SRI, GMM with instruments, first stage, weak-IV tests, etc.).\n"
            "Rules: do NOT count generic mentions of 'endogeneity' without an actual endogenous regressor in the main specs.\n"
            "Output: 1 if yes, 0 if no. Output exactly one character."
            "Part B (list): Task: If endogenous explanatory variable(s) exist in this paper, list the explanatory variable(s) "
            "explicitly treated as endogenous in the main analysis.\n"
    ...
    },
    {
        ...
        "question": (
            "Answer BOTH parts below using ONLY the provided article text.\n\n"
            "Part A (binary): Task: In the MAIN empirical analysis, do the authors implement an IV-type estimator "
            "with excluded instruments to address endogeneity?\n"
            "Look: an actual excluded instrument set used in a first stage/reduced form; 2SLS/IV/3SLS/LIML; IV-Probit; "
            "control function/2SRI; GMM with instruments; exclusion restriction; first-stage equations/tables.\n"
            "Rules: do NOT count non-statistical uses of 'instrument' (survey instrument, measurement instrument) or papers "
            "that only use RCT/RDD/DiD/event study without an IV first stage.\n"
            "Output: 1 if yes, 0 if no. Output exactly one character."
    ...
    },
    {
        ...
        "question": (
            ...
            "Look: rainfall, precipitation, drought, monsoon rainfall/onset, wet-day counts, SPI/SPEI/PDSI, "
            "precipitation-derived indices used as the EXCLUDED instrument.\n"
            "Rules: count only if precipitation-based and used as an excluded instrument in an IV first stage/reduced form. "
            "Do NOT count precipitation used only as regressor/control/interaction/exposure/outcome. "
            "Do NOT count ENSO or other climate indices unless explicitly stated to be precipitation-based AND used as the excluded instrument. "
            "Ignore mentions in other papers.\nOutput: 1 if yes, 0 if no. Output exactly one character."
            "Part B (detail): Task: List the specific rainfall/precipitation-based excluded instrument(s) used in the main IV analysis.\n"
            "Look: first-stage/reduced-form equations/tables for the precipitation-based instrument name(s) "
            "(e.g., total/mean rainfall over a window, rainfall deviations/shocks, monsoon onset, rainfall index, "
...
def build_record(row):
    """
    Build a complete multi-turn Bedrock Claude training record from one CSV row.
        ...
        with open(path, 'r', encoding='utf-8') as f:
            for i, line in enumerate(f, 1):
                ...
                assert "system" in obj, \
                    f"item {i}: missing 'system' key"
                assert len(msgs) % 2 == 0, \
                    f"item {i}: message count must be even, got {len(msgs)}"
                assert len(msgs) >= 8, \
                    f"item {i}: expected at least 8 messages (intro pair + 3 question pairs), got {len(msgs)}"

                for idx, msg in enumerate(msgs):
                    expected_role = "user" if idx % 2 == 0 else "assistant"
                    assert msg.get("role") == expected_role, \
                        f"item {i} msg[{idx}]: expected role '{expected_role}', got '{msg.get('role')}'"
                    assert msg.get("content", ""), \
                        f"item {i} msg[{idx}]: content is empty"
                    ...
                    for msg in msgs:
                        ...
                        if key_a in content:
                            assert key_b in content, \
                                f"item {i}: found '{key_a}' without '{key_b}' in assistant turn"
