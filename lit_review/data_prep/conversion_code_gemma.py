...
def build_record(row):
    """
    Build a complete multi-turn Vertex AI Gemma fine-tuning record from one CSV row.
...
def prepare_fine_tuning_data(
    ...
    """
    Reads CSV with specified columns and prepares JSONL files for Vertex AI Gemma fine-tuning.
    ...
    After generating the JSONL files, upload them to Google Cloud Storage:
      gsutil cp training_gemma.jsonl  gs://YOUR_BUCKET/training_gemma.jsonl
      gsutil cp validation_gemma.jsonl gs://YOUR_BUCKET/validation_gemma.jsonl

    Then submit the fine-tuning job with the Vertex AI SDK (see submit_finetune_gemma.py):
      import vertexai
      from vertexai.tuning import sft

      vertexai.init(project="YOUR_GCP_PROJECT", location="us-central1")
      sft_job = sft.train(
          source_model="gemma-3-27b",  # confirm exact tuning-eligible Model Garden ID in console
          train_dataset="gs://YOUR_BUCKET/training_gemma.jsonl",
          validation_dataset="gs://YOUR_BUCKET/validation_gemma.jsonl",
          tuned_model_display_name="weather-iv-gemma-v1",
          epochs=3,
          learning_rate_multiplier=1.0,
      )
      print(sft_job.tuned_model_name)  # use this endpoint in finetune_gemma.py
    """
    required_columns = [
        'filename', 'title', 'doi',
    ...
    print("\nPreview of first 3 data entries (Vertex AI Gemma format):\n")
...
training_output_path = '/Users/kieran/Library/CloudStorage/OneDrive-UniversityofArizona/weather_iv_lit/training/models/finetune_data/training_gemma.jsonl'
validation_output_path = '/Users/kieran/Library/CloudStorage/OneDrive-UniversityofArizona/weather_iv_lit/training/models/finetune_data/validation_gemma.jsonl'
