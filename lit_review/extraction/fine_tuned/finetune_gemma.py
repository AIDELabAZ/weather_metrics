...
def get_endpoint():
    ...
    Confirm the exact request schema for your deployed Gemma container in the
    Vertex AI console (Endpoints > your endpoint > Sample request) — Model Garden
    Gemma deployments commonly expose an OpenAI-compatible chat schema; adjust
    the payload in query_model_single_turn() below to match if it differs.
    """
    global _endpoint
...
def build_text_input(doc_context, prior_qa_pairs, current_key, current_question):
    """
    Assemble the full prompt for one question, identical to conversion_code_gemma.py.
