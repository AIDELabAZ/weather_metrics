...
Zero-shot baseline: identical prompts, questions, dependency logic, and PDF
extraction as extraction/fine_tuned/finetune_gpt.py, but calls the plain base
model (no fine-tuning, no RAG/reference examples). Same base checkpoint the
current fine-tuned model was tuned from, so the only variable between this
script's output and finetune_gpt.py's is the fine-tuning itself — a clean
baseline for comparison via analysis/model_eval.R.

Configured below to run against the full papers/ corpus. For a direct,
apples-to-apples accuracy comparison against the fine-tuned model specifically,
point pdf_folder at the held-out pdf_test_20 set instead — that's the subset
with ground truth in removed_20.csv.
"""
...
# Base model — the exact checkpoint the current fine-tuned model
# (ft:gpt-4.1-mini-2025-04-14:aide-lab:minjul:...) was tuned from. No
# fine-tuning, no RAG reference examples: same prompts/PDF context as
# finetune_gpt.py, only the model call differs, to isolate what fine-tuning
# actually added.
...
# -------------------------------------------------------------------
# Accuracy check helpers (identical to finetune_gpt.py)
# -------------------------------------------------------------------
...
SYNC_EVERY_N_PDFS = 50  # how often to checkpoint the local file to OneDrive mid-run


def _atomic_sync_to_onedrive(local_path, dest_path):
    """
    Copy local_path to dest_path atomically: write to a temp file in the
    SAME directory as dest_path, then os.replace() it into place. os.replace
    is an atomic rename, so OneDrive's sync client only ever observes either
...
def process_pdfs_conditional_queries(pdf_folder, output_csv):
    # Write incrementally to a LOCAL temp file, not the live OneDrive-synced
    # destination directly. A full-corpus run makes thousands of small
    # flush() calls over many hours — doing that straight into a
    # cloud-synced folder has twice now produced torn writes (rows spliced
    # together mid-write, corrupting the CSV). Only the finished file ever
    # touches the OneDrive path, copied in atomically (see
    # _atomic_sync_to_onedrive), both periodically as a crash-safety
    # checkpoint and once at the end.
    import csv as _csv
    local_tmp = os.path.join(tempfile.gettempdir(), f"baseline_gpt_{os.getpid()}.csv")
    csv_file = open(local_tmp, "w", newline="", encoding="utf-8")
    ...
    for filename in os.listdir(pdf_folder):
        ...
        n_processed += 1
        if n_processed % SYNC_EVERY_N_PDFS == 0:
            _atomic_sync_to_onedrive(local_tmp, output_csv)
            print(f"  [checkpoint] synced {n_processed} rows → {output_csv}")
...
# /output/gpt/baseline/test_baseline_gpt_output.csv while if running full extraction specify pdf_folder to /weather_iv_lit/papers and output to
# /output/gpt/baseline/full_baseline_gpt_output.csv

if __name__ == "__main__":
    pdf_folder = ""
    output_folder = ""
    os.makedirs(output_folder, exist_ok=True)
    output_csv = os.path.join(output_folder, "full_baseline_gpt_output.csv")
