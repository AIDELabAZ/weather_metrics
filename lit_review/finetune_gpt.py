...
questions = [
    ...
    },
    {
        ...
        "question": "Task: Extract the primary dependent (outcome) variable(s) in the article's main regression model. What to extract: Only the main dependent variable(s) of the primary regression specification. Steps: first find all phrases in this text that explicitly describe the main outcome or dependent variable of the empirical analysis. Second, from these phrases, deduce and select the main dependent variable(s) used in the analysis and normalize each to a short descriptive name. Rules: Do not output single letters, Greek letters, or purely symbolic notation as dependent variable names. Do not output internal dataset or code variable names that have no clear descriptive meaning on their own. Output format: Return the main dependent variable name. If there is one main outcome, output just that one. If there are several distinct main dependent variables, output up to three, in a semicolon-separated list on a single line. Do not repeat the same name twice. If the article does not estimate any regression or statistical model with a clearly defined outcome variable in the main analysis, output exactly n/a. Output exactly one line with this content and nothing else."
    },
    {
        "key": "Endogenous Variable(s)",
        "question": "Task: Extract the primary explanatory variable(s) the authors explicitly treat as endogenous in the main empirical analysis. This is the variable that is instrumented for if an instrumental variables technique is used. What to extract: Only the main endogenous explanatory variable(s) of the primary regression specification. These are regressors whose endogeneity motivates the identification strategy and are described as being instrumented. Rules: Do not output single letters, Greek letters, or purely symbolic notation as variable names. If multiple distinct endogenous variables are used, include each variable once. Use only this article's own content. Output Format: Return the name of the primary endogenous variable(s). Return only the name(s) of the endogenous variable(s), with no extra text. If there are several distinct independent variables, separate names with semicolons and output no more than three. Do not include any non-endogenous variables and do not repeat the same name twice. Your final output must be exactly one line containing the endogenous variable name(s) with nothing else."
    },
    {
