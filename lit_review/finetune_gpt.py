...
questions = [
    ...
    },
    {
        ...
        "question": "Task: Extract the dependent (outcome) variable(s) are in the article's main regression model(s). What to extract: The dependent variable name as it appears on the left-hand side of the main regression equation or as the column header or label in the main regression tables. If the same dependent variable is used in multiple specifications or samples, include it only once. Clean the name by removing transformations and units: if the dependent variable is written as ln(wage), log wage, log(wage), log income, Δ GDP, or similar, output the underlying quantity such as wage, income, or gdp. Remove units and clarifying parentheses such as (per 1,000), (in 2015 USD), (kg/ha), (monthly), or (per capita). Keep a short, descriptive name that identifies the outcome (for example: maize yield → maize yield; child stunting → child stunting). Output format: Return only the dependent variable name or, if there are several distinct main dependent variables, a semicolon-separated list on a single line, all in lower case, with a single space after each semicolon, for example: wage; employment; child stunting. Do not repeat the same name twice. Do not include any variable that is not a dependent variable. Output exactly one line with this content and nothing else"
    },
    {
