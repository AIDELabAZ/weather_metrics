...
questions = [
    {
        "key": "Paper Title",
        "question": "Task: Extract the exact article title. What to extract: The main standalone title of the article as it appears in the document, including any subtitle that is part of the same heading. How to extract and clean: Look for the main heading that appears before the author names and/or abstract. Ignore running headers, journal names, volume/issue info, page numbers, series names, and later section headings (for example, “1 Introduction”). Do not include author names or affiliations, journal info, section titles (for example, “Abstract”, “Introduction”), or any footnote markers or symbols attached to the title. If there is a title and subtitle in one heading (for example, separated by a colon or dash), include the full combined text. Preserve the original wording, spelling, punctuation, and capitalization, except remove footnote markers. Output format: Output exactly one line containing only the title text. Do not output quotes, labels, or any other text. Assume there is exactly one title. Perform all reasoning internally; do not describe it."
    },
    {
        "key": "DOI",
        "question": "Task: Extract the DOI of the focal article (version of record). What to extract: The canonical DOI string (for example, 10.1016/j.jpubeco.2020.104123) for the published article. How to extract and clean: Search for DOI-like strings, especially near the front matter, headers/footers, citation blocks, or journal info, and near labels like DOI:, doi:, or https://doi.org/. Ignore DOIs that appear only in the reference list, unless they clearly correspond to a citation block belonging to this paper rather than cited works. If multiple DOIs appear, choose the one that matches this article title, authors, journal, and year and corresponds to the published version (not a preprint or dataset/supplement). Prefer the full publisher DOI over any shortDOI. Strip URL prefixes (doi:, DOI:, https://doi.org/, http://dx.doi.org/), remove spaces and hyphenation from line breaks, and remove trailing punctuation such as periods or commas. Convert the DOI to lowercase. Output format: Output exactly one token: either the normalized DOI string or exactly n/a if no DOI exists for the focal article. No extra text, spaces, or line breaks beyond that single line. Perform all reasoning internally; do not describe it."
    },
    {
