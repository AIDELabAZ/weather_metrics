...
questions = [
    {
        "key": "Paper Title",
        "question": "Task: Extract the exact article title. What to extract: Read the first page of an academic article and return only its exact title. The title is the main standalone heading that names the article itself, not the journal, series, running header, section title, or author list. It usually appears once near the top of the first page and directly precedes the author names or abstract. Exclude all author names, affiliations, journal information, page numbers, section headings (such as “Abstract” or “Introduction”), and any footnote markers or symbols. If the title contains a subtitle in the same heading (for example separated by a colon or dash), include the full combined text. Preserve the original wording, spelling, punctuation, and capitalization, except remove footnote markers. Output format: Output exactly one line containing only the title text and nothing else."
    },
    {
        "key": "DOI",
    ...
    },
    {
        ...
        "question": "Task: Extract the main dependent (outcome) variable or variables in the article's primary regression model. What to extract: Only the main dependent variable(s) of the primary regression specification, not every outcome mentioned in the text. These are the outcomes whose coefficients are emphasized as the main results. They are usually described in the empirical model section and appear as the left-hand-side variable or as the first column header(s) in the main regression table. Steps: first find all phrases in this text that explicitly describe the main outcome or dependent variable of the empirical analysis (e.g., childhood mortality, maize yield, test scores). Second, from these phrases, select the main dependent variable(s) and normalize each to a short descriptive name in lower case; output a semicolon-separated list or n/a. Rules: Do not output single letters, Greek letters, or purely symbolic notation (like y, c, z, β, φ, ε, u, v, i, n) as dependent variable names. Do not output internal dataset or code variable names that have no clear descriptive meaning on their own. Output format: Return at most three main dependent variable names. If there is one main outcome, output just that one. If there are several distinct main dependent variables, output up to three, in a semicolon-separated list on a single line, lower case, with a single space after each semicolon (for example: civil conflict incidence; civil conflict onset). Do not repeat the same name twice. If the article does not estimate any regression or statistical model with a clearly defined outcome variable in the main analysis, output exactly n/a. Output exactly one line with this content and nothing else."
    },
    {
