    ...
    try:
        ...
        if iv_used == 'yes':
            ...
            for detail in iv_details_text:
                if detail.startswith("Endogenous variable:"):
                    iv_details["Endogenous variable"] = detail.replace("Endogenous variable:", "").strip()
            ...
            if any(method in iv_details["Instrumental variable"].lower() for method in rainfall_methods):
                ...
                if any(method in specific_rainfall_metric.lower() for method in rainfall_methods):
                    iv_details["Instrumental variable"] = specific_rainfall_metric
                else:
                    iv_details["Instrumental variable"] = "N/A"

                rainfall_as_iv = "yes"
        
        return title, doi, iv_used, rainfall_as_iv, iv_details["Endogenous variable"], iv_details["Instrumental variable"], iv_details["Dependent variable"], iv_details["Control variables"]
...
def main():
    ...
    with open(csv_output_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        writer.writerow(['Filename', 'Title', 'DOI', 'IV Used', 'Rainfall as IV', 'Endogenous Variable', 'Instrumental Variable', 'Dependent Variable', 'Control Variables'])
